...
const btnHistory = document.getElementById('btn-history-search'); // Ref do Nãovo botão
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // O loadAndDisReproduzirChats() será chamado apenas por handleWhatsappPronto após conexão bem-sucedida.

    // Atualiza Listaa de clientes ao iniciar
    ...
    const currentUser = 'Operador'; // Ajustar: recuperar do Entrar se houver
...
// Botão para adicionar Nãovo WhatsApp
...
// Listaener para Nãovos clientes conectados
window.whatsappAPI && window.whatsappAPI.onNewClientReady && window.whatsappAPI.onNewClientReady((data) => {
    ...
    // Atualiza a Listaa de clientes
    updateConnectedClientsList();
    
    // Mostra Nãotificação visual
...
function updateConnectionStatus(connected) {
    ...
    // 🚨 chamada a renderConversationsLista() removida intencionalmente
}
...
// Função para atualizar a Listaa de clientes conectados
...
// Torna a função global para uso Não onclick do HTML
